...
**De novo binder design**, a different and more ambitious method: RFdiffusion's protein-protein-interaction mode generates new binder backbones directly against NDM-1's dizinc active site (hotspot-guided on the real, UniProt-verified zinc-coordinating residues), ProteinMPNN designs each binder's sequence with the target chain held completely fixed, and Chai-1 folds the resulting complex to get a real interface-confidence score (ipTM) -- a fundamentally different question than repurposing: not "does an existing drug happen to fit," but "can a new protein be designed to bind this target at all." 150 backbones, 600 sequences, folded overnight, unattended, auto-chained between stages. Raw result: 255/600 above ipTM 0.5. That number got real scrutiny before being trusted -- a hypothesis that low-complexity, alanine-heavy sequences were inflating scores was tested across all 600 (not just a small sample) and *not* supported by the data (weak, wrong-direction correlation), so that specific concern was retracted rather than left standing. A second, independent, purely geometric check -- does the binder structurally reach the real catalytic hotspot, not just score well -- found 116/255 candidates genuinely contact the hotspot region within 5 Å (closest: 1.7-2.8 Å). A third check closed the last gap: does the binder's predicted structure actually match what RFdiffusion designed, not just something Chai-1 converged on independently (the same self-consistency question the enzyme pipeline elsewhere in this repo always asks). The first attempt at that check hit the exact same residue-numbering bug this campaign had already been caught on twice before -- caught before being trusted, fixed, rerun. Corrected result: 48/116 candidates pass all three independent checks (ipTM > 0.5, real hotspot contact, backbone RMSD < 2.0 Å). That triple-verified set of 48 is the real result. A follow-up ablation then tested the remaining open question directly: does raising ProteinMPNN's sampling temperature (0.5 and 1.0, vs. the original 0.2) fix the low-complexity, alanine-heavy composition. It does -- cleanly, monotonically (entropy rises from ~2.8 to 3.7 bits, alanine content drops from ~44% toward 21% at temp 1.0) -- but at a steep, measured cost: mean ipTM drops from 0.446 to 0.208, and the >0.5 hit rate falls 8.5x. A real, quantified tradeoff, not a free fix, and not resolved in favor of either setting. Full writeup, including the retracted hypothesis, both numbering bugs, and the temperature ablation, in `research/ndm1_binder_design.md`.
